...
欢迎来到 |product-name| 指南。|product-name| 是一个高性能，完全托管的PB级数据仓库服务。一个 |product-name| 是由一组称之为节点的计算资源组成的集群。每个集群作为一个 |product-name| 引擎，包含一个或多个数据库。

这个指南的目的是指导你如何创建一个 |product-name| 样例集群。你可以通过这个样例集群来测试 |product-name| 的功能。在这个教程中，你将执行如下步骤：

-  步骤1：准备工作
-  步骤2：启动 |product-name| 样例集群
-  步骤3：授权连接样例集群
-  步骤4：连接样例集群
-  步骤5：将样例数据从对象存储加载到 |product-name| 中
-  步骤6：寻找额外的资料和重设你的环境
...
在你创建第一个 |product-name| 集群前，确保在这个章节中完成如下准备工作：
...
你可以使用任何 |postgres| 兼容的客户端程序连接到 |product-name|，比如 psql 。此外，你还可以通过绝大部分使用标准数据库应用接口，如 JDBC，ODBC 的客户端程序连接到 |product-name|。最后，你还可以使用标准数据库应用接口开发自己的客户端程序来访问 |product-name|。由于 |product-name| 基于 |greenplum|，而后者又是基于 |postgres| 而来，所以你可以直接使用 |postgres| 驱动访问 |product-name|。在这个教程中，我们将通过 psql 这个 |postgres| 的客户端程序演示如何连接到 |product-name|。
...
2. 或者访问 `PostgreSQL 官方网站 <http://postgresql.org>`__，根据你的操作系统下载安装包。

确认网络配置
--------------

|product-name| 使用5432作为服务的端口地址。
当你希望从云的外部访问 |product-name| 服务时，
你不仅需要拥有一个公网IP地址，
还需要指定一个绑定在公网IP地址上的端口，
并将此端口转发到 |product-name| 的服务端口5432上。
我们在随后的章节中帮助你完成端口转发的工作，
现在你只需要明确这个端口的地址即可。
你可以随意指定这个端口，
为了便于记忆，我们推荐你仍然使用5432作为公网端口地址。
...
完成上面的前提步骤后，现在你能够开始启动一个 |product-name| 集群。
...
在前面的步骤中，你已经创建启动了你的 |product-name| 集群。在你连接到数据仓库集群之前，你需要对上一步骤中创建的路由器进行相应的配置。
...
.. image:: ../_static/qingcloud/firewall.png
    :width: 100%

这条下行规则允许你的SQL客户端工具能够访问路由器的 5432 端口。
...
.. image:: ../_static/qingcloud/vpc_port_forward.png
   :width: 100%

这条转发规则确保在你访问路由器 5432 端口时，请求转发到 |product-name| 集群的主节点 5432 端口。
...
1. 创建表
   ...
   拷贝并执行下面的建表语句在 postgres 数据库中创建相应的表对象。你可以通过 |product-name| \ `开发者指南 <http://www.hashdata.cn/docs/developer-guide/welcome.html>`__\ 查看更详细的建表语法。
...
-  |product-name| \ `集群管理指南 <http://www.hashdata.cn/docs/cluster-management-guide/cluster-management-guide.html>`__\
-  |product-name| \ `数据库开发者指南 <http://www.hashdata.cn/docs/developer-guide/welcome.html>`__\
